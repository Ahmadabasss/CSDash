...
const STATUS_STYLE: Record<string, string> = {
  Unhealthy: 'bg-red-100 text-red-700 ring-1 ring-red-200',
  Healthy: 'bg-emerald-100 text-emerald-700 ring-1 ring-emerald-200',
  NotApplicable: 'bg-[#edebe9] text-[#605e5c]',
  NotApplicableInformational: 'bg-[#edebe9] text-[#605e5c]',
}
...
export default async function RecommendationDetailPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  return (
    <div className="px-6 py-6 max-w-4xl">
      <Link href="/recommendations" className="inline-flex items-center gap-1.5 text-sm text-[#605e5c] hover:text-[#323130] mb-6 transition-colors">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-semibold text-[#323130] leading-snug">{props.displayName}</h1>
          <p className="text-xs text-[#797775] font-mono mt-1">{rec.name}</p>
        </div>
        <span className={`text-xs font-medium px-2.5 py-1 rounded shrink-0 ${STATUS_STYLE[statusKey] ?? 'bg-[#edebe9] text-[#605e5c]'}`}>
          {isHealthy
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          ...
          <Section title="Description">
            <p className="text-[#4b4b4b] text-sm leading-relaxed">{meta.description}</p>
          </Section>
          ...
          <Section title="Remediation Steps">
            ...
            <div className="flex gap-3 mb-4">
              <div className="flex items-center gap-2 rounded-lg bg-white/60 px-3 py-2 text-xs ring-1 ring-[#edebe9]">
                <Zap className="w-3.5 h-3.5 text-[#605e5c]" />
                <span className="text-[#605e5c]">Effort:</span>
                <span className={`font-semibold ${EFFORT_COLOR[meta.implementationEffort] ?? 'text-[#4b4b4b]'}`}>{meta.implementationEffort}</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-white/60 px-3 py-2 text-xs ring-1 ring-[#edebe9]">
                <Users className="w-3.5 h-3.5 text-[#605e5c]" />
                <span className="text-[#605e5c]">User impact:</span>
                <span className={`font-semibold ${IMPACT_COLOR[meta.userImpact] ?? 'text-[#4b4b4b]'}`}>{meta.userImpact}</span>
              </div>
              <div className="flex items-center gap-2 rounded-lg bg-white/60 px-3 py-2 text-xs ring-1 ring-[#edebe9]">
                <Clock className="w-3.5 h-3.5 text-[#605e5c]" />
                <span className="text-[#605e5c]">Status:</span>
                <span className={`font-semibold ${isHealthy ? 'text-emerald-600' : 'text-red-600'}`}>{statusKey}</span>
              </div>
            </div>

            {status.cause && (
              <div className="flex items-start gap-2 mb-4 bg-amber-100 border border-amber-800/40 rounded p-3">
                <AlertTriangle className="w-4 h-4 text-amber-600 shrink-0 mt-0.5" />
                <p className="text-xs text-amber-700">{status.cause}</p>
              </div>
            )}
            ...
            <a
              ...
              className="mt-4 inline-flex items-center gap-1.5 text-xs text-[#0078d4] hover:text-sky-300 transition-colors"
            >
            ...
            <Section title="Threat Coverage">
              <div className="flex flex-wrap gap-2">
                {meta.threats.map(threat => (
                  <span key={threat} className="flex items-center gap-1.5 text-xs bg-red-100 text-red-700 border border-red-800/40 px-2.5 py-1.5 rounded">
        ...
        <div className="space-y-4">
          ...
          <Section title="Metadata">
            <dl className="space-y-3">
              ...
              {meta.categories.length > 0 && (
                <div>
                  <dt className="text-xs text-[#797775] flex items-center gap-1 mb-1.5">
          ...
          {status.description && (
            <Section title="Status Detail">
              <p className="text-xs text-[#605e5c] leading-relaxed">{status.description}</p>
            </Section>
          )}
        </div>
      </div>
...
function RemediationSteps({ text }: { text: string }) {
  ...
  if (steps.length <= 1) {
    return <p className="text-[#4b4b4b] text-sm leading-relaxed">{text}</p>
  }
    ...
    <ol className="space-y-3">
      {steps.map((step, i) => (
        <li key={i} className="flex gap-3">
          ...
          <p className="text-[#4b4b4b] text-sm leading-relaxed">{step}</p>
        </li>
      ))}
    </ol>
  )

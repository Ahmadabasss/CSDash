...
export default async function ReportsPage() {
  ...
  try {
    ...
    return (
      <div className="p-8 text-slate-400">Backend offline — start the FastAPI server on port 8000.</div>
    )
  }
  ...
  return (
    <div className="px-6 py-6 max-w-5xl">
      ...
      <div className="flex items-center justify-between mb-6 print:hidden">
        <div>
          <h1 className="text-xl font-semibold text-white">Executive Security Report</h1>
          <p className="text-sm text-slate-400 mt-0.5">Point-in-time summary · {generatedAt}</p>
        </div>
        <PrintButton />
      </div>
      ...
      <ReportSection title="Secure Score" icon={<ShieldCheck className="w-4 h-4 text-emerald-400" />}>
      ...
      {/* ── Section 2: Alerts ── */}
      <ReportSection title="Active Alerts" icon={<Bell className="w-4 h-4 text-red-400" />}>
        <div className="grid grid-cols-3 gap-4">
          <BigStat label="Total Open" value={summary.openAlerts.total} color="text-red-400" />
          <BigStat label="High Severity" value={summary.openAlerts.high} color="text-red-400" />
          <BigStat label="Medium Severity" value={summary.openAlerts.medium} color="text-amber-400" />
        </div>
        <p className="mt-4 text-sm text-slate-300 leading-relaxed">
      ...
      <ReportSection title="Recommendations" icon={<ListChecks className="w-4 h-4 text-amber-400" />}>
        <div className="grid grid-cols-3 gap-4">
          <BigStat label="Unhealthy" value={summary.unhealthyRecommendations.total} color="text-amber-400" />
          <BigStat label="High Severity" value={summary.unhealthyRecommendations.high} color="text-red-400" />
          <BigStat label="Medium Severity" value={summary.unhealthyRecommendations.medium} color="text-amber-400" />
        </div>
        <p className="mt-4 text-sm text-slate-300 leading-relaxed">
      ...
      <ReportSection title="Vulnerabilities" icon={<Bug className="w-4 h-4 text-orange-400" />}>
        <div className="grid grid-cols-3 gap-4">
          <BigStat label="Total CVEs" value={vulns.length} />
          <BigStat label="Exploitable" value={exploitableVulns.length} color={exploitableVulns.length > 0 ? 'text-red-400' : 'text-emerald-400'} />
          <BigStat label="Critical (CVSS ≥9)" value={criticalVulns.length} color={criticalVulns.length > 0 ? 'text-red-400' : 'text-emerald-400'} />
        </div>
        {exploitableVulns.length > 0 && (
          <div className="mt-4 space-y-1.5">
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2">Top Exploitable CVEs</p>
            {exploitableVulns.slice(0, 5).map(v => (
              <div key={v.id} className="flex items-center justify-between text-sm bg-slate-900/60 rounded px-3 py-2">
                <span className="font-mono text-red-400 text-[12px]">{v.id}</span>
                <span className="text-slate-400 text-xs">CVSS {v.cvssV3}</span>
              </div>
            ))}
          </div>
        )}
      ...
      <ReportSection title="Compliance Posture" icon={<ShieldCheck className="w-4 h-4 text-blue-400" />}>
          ...
          {compliance.map(s => {
            ...
            return (
              <div key={s.id} className="flex items-center gap-4">
                <span className="text-sm text-slate-300 w-40 shrink-0 truncate">{s.name}</span>
                ...
                <span className={`text-sm font-semibold tabular-nums w-12 text-right shrink-0 ${passPct >= 70 ? 'text-emerald-400' : passPct >= 40 ? 'text-amber-400' : 'text-red-400'}`}>
                  {passPct}%
                </span>
      ...
      <ReportSection title="Identity & Endpoints" icon={<Users className="w-4 h-4 text-purple-400" />}>
        <div className="grid grid-cols-2 gap-6">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">Risky Users</p>
            <div className="grid grid-cols-2 gap-3">
              <BigStat label="At Risk" value={riskyUserSummary.atRisk} color="text-amber-400" small />
              <BigStat label="Compromised" value={riskyUserSummary.confirmedCompromised} color="text-red-400" small />
            </div>
          </div>
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-3">Endpoints (MDE)</p>
            <div className="grid grid-cols-2 gap-3">
              <BigStat label="High / Critical" value={endpointSummary.highRisk} color="text-red-400" small />
              <BigStat label="Total CVEs" value={endpointSummary.totalVulnerabilities} color="text-amber-400" small />
            </div>
          </div>
        </div>
      </ReportSection>
      ...
      {/* ── Footer ── */}
      <div className="mt-8 pt-4 border-t border-white/6 print:border-gray-200 flex items-center justify-between text-xs text-slate-600">
...
function ReportSection({ title, icon, children }: { title: string; icon: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="mb-6 bg-[#1e293b] print:bg-white border border-white/6 print:border-gray-200 rounded-xl p-6 break-inside-avoid">
      <div className="flex items-center gap-2 mb-5">
        <span className="print:hidden">{icon}</span>
        <h2 className="text-sm font-semibold uppercase tracking-wider text-slate-400 print:text-gray-600">{title}</h2>
      </div>
      {children}
    </div>
  )

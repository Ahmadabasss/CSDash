...
export default async function ResourceDetailPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  return (
    <div className="px-6 py-6 max-w-5xl">
      <Link href="/resources" className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-200 mb-6 transition-colors">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1 min-w-0">
          <h1 className="text-xl font-bold font-mono text-white">{resource.name}</h1>
          <p className="text-sm text-slate-400 mt-0.5">{formatResourceType(resource.type)}</p>
        </div>
        <div className={`text-center bg-[#1e293b] border border-white/6 rounded-lg px-4 py-2`}>
          <p className={`text-2xl font-bold tabular-nums ${SCORE_COLOR(resource.secureScore)}`}>{resource.secureScore}</p>
          <p className="text-[10px] text-slate-500 mt-0.5">Secure Score</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
        ...
        <StatCard
          ...
          color={resource.issuesCount > 5 ? 'text-red-400' : resource.issuesCount > 0 ? 'text-amber-400' : 'text-emerald-400'}
        />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
            ...
            <Section title={`Related Alerts (${resource.relatedAlerts!.length})`}>
              <div className="space-y-2">
                {resource.relatedAlerts!.map(alert => (
                  <div key={alert.id} className="flex items-center gap-3 bg-slate-900/60 rounded px-3 py-2.5">
                    <Bell className="w-4 h-4 text-slate-500 shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] text-slate-200 font-medium truncate">{alert.title}</p>
                      <p className="text-[11px] text-slate-500">{alert.category}</p>
                    </div>
            ...
            <Section title={`Recommendations (${resource.relatedRecommendations!.length})`}>
              <div className="space-y-2">
                {resource.relatedRecommendations!.map(rec => (
                  <div key={rec.id} className="flex items-center gap-3 bg-slate-900/60 rounded px-3 py-2.5">
                    <Shield className="w-4 h-4 text-slate-500 shrink-0" />
                    <div className="flex-1 min-w-0">
                      <p className="text-[12px] text-slate-200 font-medium truncate">{rec.properties.displayName}</p>
                      <p className="text-[11px] text-slate-500">{rec.properties.status.code}</p>
                    </div>
          ...
          {!hasAlerts && !hasRecs && (
            <div className="bg-[#1e293b] border border-white/6 rounded-lg p-8 text-center">
              <Shield className="w-8 h-8 text-slate-600 mx-auto mb-2" />
              <p className="text-slate-500 text-sm">No related alerts or recommendations</p>
            </div>
          )}
        </div>
        ...
        <div className="space-y-4">
          <Section title="Resource Details">
            <dl className="space-y-2.5">
              ...
              <div>
                <dt className="text-xs text-slate-500">Resource Type</dt>
                <dd className="text-xs text-slate-300 mt-0.5 break-all font-mono">{resource.type}</dd>
              </div>
            </dl>
          </Section>

...
const TAG_STYLE: Record<string, string> = {
  EXPLOIT_AVAILABLE: 'bg-red-900/60 text-red-200 ring-1 ring-red-700',
  RANSOMWARE_ASSOCIATED: 'bg-orange-900/60 text-orange-200 ring-1 ring-orange-700',
  ACTIVE_THREAT: 'bg-red-900 text-red-100 ring-1 ring-red-600',
  WORM: 'bg-amber-900/60 text-amber-200 ring-1 ring-amber-700',
}
...
export default async function CVEDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="px-6 py-6 max-w-4xl">
      <Link href="/vulnerabilities" className="inline-flex items-center gap-1.5 text-sm text-slate-400 hover:text-slate-200 mb-6 transition-colors">
      ...
      <div className="flex items-start gap-4 mb-6">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-slate-300 mt-1 leading-snug">{cve.name}</p>
        </div>
        <a
          href={`https://nvd.nist.gov/vuln/detail/${cve.id}`}
          target="_blank"
          rel="noopener noreferrer"
          className="flex items-center gap-1.5 text-xs text-[#60a5fa] hover:text-white transition-colors bg-[#1e293b] border border-white/6 px-3 py-1.5 rounded shrink-0"
        >
      ...
      <div className="bg-[#1e293b] border border-white/6 rounded-lg p-5 mb-4">
        <div className="flex items-center gap-8">
          <div className="text-center">
            ...
            <p className="text-xs text-slate-500 mt-1">CVSS v3 Base Score</p>
          ...
          <div className="flex-1">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-xs text-slate-400 w-12">/ 10.0</span>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          ...
          <Section title="Description">
            <p className="text-slate-300 text-sm leading-relaxed">{cve.description}</p>
          </Section>
          ...
          <Section title="Exploit Information">
            ...
            {cve.exploitTypes.length > 0 && (
              <div className="mt-3">
                <p className="text-xs text-slate-500 uppercase tracking-wider mb-2">Exploit Types</p>
                <div className="flex flex-wrap gap-2">
                  {cve.exploitTypes.map(t => (
                    <span key={t} className="text-xs bg-slate-700/60 text-slate-300 px-2.5 py-1 rounded font-mono">{t}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          {cve.tags.length > 0 && (
            <Section title="Threat Intelligence Tags">
              <div className="flex flex-wrap gap-2">
                {cve.tags.map(tag => (
                  <span key={tag} className={`text-xs font-semibold px-2.5 py-1.5 rounded ${TAG_STYLE[tag] ?? 'bg-slate-700 text-slate-300'}`}>
                    {tag.replace(/_/g, ' ')}
                  </span>
                ))}
              </div>
        ...
        <div className="space-y-4">
          <Section title="Impact">
            <dl className="space-y-3">
              <div>
                <dt className="text-xs text-slate-500">Exposed Machines</dt>
                <dd className={`text-2xl font-bold mt-0.5 ${cve.exposedMachines > 0 ? 'text-red-400' : 'text-slate-400'}`}>
                  {cve.exposedMachines}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500">CVE Severity</dt>
          ...
          <Section title="Dates">
            <dl className="space-y-2.5">
              <div>
                <dt className="text-xs text-slate-500 flex items-center gap-1"><Calendar className="w-3 h-3" /> Published</dt>
                <dd className="text-xs text-slate-300 mt-0.5">{new Date(cve.publishedOn).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</dd>
              </div>
              <div>
                <dt className="text-xs text-slate-500 flex items-center gap-1"><RefreshCw className="w-3 h-3" /> Last Updated</dt>
                <dd className="text-xs text-slate-300 mt-0.5">{new Date(cve.updatedOn).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</dd>
              </div>
            </dl>
          </Section>
        </div>
...
function ExploitCard({ label, active, activeText, inactiveText, danger }: {
  ...
  return (
    <div className={`rounded p-3 border ${active && danger ? 'bg-red-900/20 border-red-800/40' : 'bg-slate-900/60 border-white/4'}`}>
      <p className="text-xs text-slate-500">{label}</p>
      <p className={`text-sm font-semibold mt-1 ${active && danger ? 'text-red-400' : 'text-slate-400'}`}>

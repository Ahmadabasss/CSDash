...
const RISK_COLOR = {
  high:   { fill: '#7f1d1d', stroke: '#ef4444', text: '#fca5a5', badge: 'bg-red-900/60 text-red-300' },
  medium: { fill: '#431407', stroke: '#f59e0b', text: '#fcd34d', badge: 'bg-amber-900/60 text-amber-300' },
  low:    { fill: '#172554', stroke: '#60a5fa', text: '#93c5fd', badge: 'bg-blue-900/40 text-blue-300' },
  none:   { fill: '#1e293b', stroke: '#64748b', text: '#94a3b8', badge: 'bg-slate-700 text-slate-400' },
} as const
    ...
    <div ref={containerRef} className="relative select-none">
      ...
      >
        <defs>
          ...
          <radialGradient id="br-bg" cx="50%" cy="50%" r="50%">
        ...
        {/* Orbit rings */}
        {innerNodes.length > 0 && (
          <circle cx={CX} cy={CY} r={INNER_R} fill="none" stroke="#ef444420" strokeWidth="1.5" strokeDasharray="6 10" />
        )}
        {outerNodes.length > 0 && (
          <circle cx={CX} cy={CY} r={OUTER_R} fill="none" stroke="#33415516" strokeWidth="1" strokeDasharray="4 10" />
        )}
        {data.affectedUsers.length > 0 && (
          <circle cx={CX} cy={CY} r={USER_R} fill="none" stroke="#4c1d9530" strokeWidth="1" strokeDasharray="2 7" />
        )}
        ...
        {data.affectedUsers.map((u, i) => {
          ...
          return (
            <line key={u.userPrincipalName + '-l'} x1={CX} y1={CY} x2={p.x} y2={p.y}
              stroke="#6d28d936" strokeWidth="0.7" strokeDasharray="2 5"
              className="br-du"
            />
          )
        })}
        ...
        >
          ...
          {/* thin score arc placeholder ring */}
          ...
          return (
            ...
            >
              ...
              <text x={p.x} y={p.y + nodeR + 15} textAnchor="middle"
                fill={sel ? '#e2e8f0' : hov ? '#cbd5e1' : '#94a3b8'} fontSize="7.5"
        ...
        {outerNodes.map((n, i) => {
          ...
          return (
            ...
            >
              {(sel || hov) && (
                <circle cx={p.x} cy={p.y} r={27} fill="none" stroke="#94a3b8"
                  strokeWidth={sel ? 1.5 : 1}
              ...
              <circle cx={p.x} cy={p.y} r={18}
                fill={hov ? '#1e3a5f' : sel ? '#1e3a5f' : '#1e293b'}
                stroke={sel || hov ? '#94a3b8' : '#475569'}
                strokeWidth={sel ? 2 : hov ? 1.5 : 1}
                filter={(sel || hov) ? 'url(#br-glow-sm)' : undefined}
                style={{ transition: 'fill 0.15s, stroke-width 0.15s' }}
              />
              <text x={p.x} y={p.y + 4} textAnchor="middle"
                fill={sel || hov ? '#e2e8f0' : '#94a3b8'} fontSize="8" fontFamily="monospace">
                {shortType(n.type)}
              </text>
              <text x={p.x} y={p.y + 29} textAnchor="middle"
                fill={sel || hov ? '#94a3b8' : '#64748b'} fontSize="7">
                {label}
              </text>
            </g>
          )
      ...
      {hover && (
        ...
        >
          <div className="rounded-xl bg-slate-900/95 ring-1 ring-white/10 shadow-2xl px-3.5 py-3 text-xs backdrop-blur-sm">
            {hover.kind === 'center' && (
              <>
                <div className="flex items-center gap-2 mb-2">
                  <span className="rounded px-1.5 py-0.5 bg-red-900/60 text-red-300 font-mono text-[10px] font-bold">TARGET</span>
                  <span className="text-slate-400 font-mono text-[10px]">{hover.subtype}</span>
                </div>
                <p className="font-semibold text-white text-[12px] leading-snug mb-2 break-all">{hover.name}</p>
              ...
              <>
                <div className="flex items-center gap-2 mb-2">
                  <span className={`rounded px-1.5 py-0.5 text-[10px] font-semibold capitalize
                    ${hover.risk === 'high' ? 'bg-red-900/60 text-red-300' :
                      hover.risk === 'medium' ? 'bg-amber-900/60 text-amber-300' :
                      hover.risk === 'low' ? 'bg-blue-900/50 text-blue-300' :
                      'bg-slate-700 text-slate-400'}`}>
                    {hover.risk ?? 'low'} risk
                  </span>
                  <span className="text-slate-500 font-mono text-[10px]">{hover.subtype}</span>
                </div>
                <p className="font-semibold text-white text-[12px] leading-snug mb-2 break-all">{hover.name}</p>
                <div className="space-y-1 text-[11px]">
                ...
                <p className="mt-2 pt-2 border-t border-white/8 text-slate-600 text-[10px]">Click to inspect</p>
              </>
            )}
            ...
            {hover.kind === 'user' && (
              <>
                <div className="flex items-center gap-1.5 mb-2">
                  <span className="rounded px-1.5 py-0.5 bg-purple-900/60 text-purple-300 text-[10px] font-semibold">User</span>
                </div>
                <p className="font-mono text-white text-[11px] leading-snug mb-2 break-all">{hover.name}</p>
      ...
      {/* Legend */}
      <div className="flex flex-wrap items-center gap-x-5 gap-y-1.5 mt-3 text-[11px] text-slate-500">
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-red-900 ring-1 ring-red-500 shrink-0" />Target
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-red-950 ring-1 ring-red-400 shrink-0" />Shared alert
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-slate-800 ring-1 ring-slate-500 shrink-0" />Same RG
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-purple-950 ring-1 ring-purple-600 shrink-0" />Affected user
        </span>
        <span className="ml-auto text-slate-600 italic text-[10px]">Hover to preview · Click to inspect</span>
      </div>
    </div>
  )
}
...
function Row({ label, value, highlight }: { label: string; value: string; highlight?: boolean }) {
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className={`font-medium ${highlight ? 'text-amber-300' : 'text-slate-300'}`}>{value}</span>
    </div>
  )
}
